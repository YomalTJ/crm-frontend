...
const ViewBeneficiaries = () => {
  ...
  const handleCreateBusinessEmpowerment = (familyId: string) => {
    router.push(`/dashboard/gn-level/beneficiary-training/create/${familyId}`);
  };
  ...
  if (loading) {
    return (
      <div className={`flex items-center justify-center min-h-screen ${getBgColor()}`}>
        ...
        <span className={`ml-3 text-lg ${getTextColor()}`}>Loading beneficiaries data...</span>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Header with Filters */}
      <PageHeader
        title="View All Beneficiaries for Beneficiary Training"
      ...
      <ProjectDataTable
        data={filteredData.map(item => ({
          ...
          action: (
            <button
              onClick={() => handleCreateBusinessEmpowerment(item.nic || item.hh_number)}
