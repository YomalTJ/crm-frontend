...
const FamilyAgeGroups = () => {
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                ...
                // Set the initial filters to user's default location with 16-24 age range
                const initialFilters = { ...defaultFilters, minAge: 16, maxAge: 24 };
                setFilters(initialFilters);

                // Load family demographics with user's default location filters
    ...
    const updateFilter = (key: string, value: string) => {
        ...
        if (value === '') {
            delete newFilters[key as keyof FamilyDemographicsFilters];
        } else {
            if (key === 'mainProgram') {
                if (value === 'NP' || value === 'ADB' || value === 'WB') {
                    newFilters[key as keyof FamilyDemographicsFilters] = value as any;
                }
            } else {
                newFilters[key as keyof FamilyDemographicsFilters] = value as any;
            }
