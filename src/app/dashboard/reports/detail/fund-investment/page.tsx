...
import {
    getFundInvestmentSummary,
    getUserDefaultLocationForFundInvestment,
    FundInvestmentSummary,
    FundInvestmentFilters
} from '@/services/fundInvestmentService';
import {
...
import FundInvestmentTable from '@/components/form-fields/FundInvestmentTable';
import { formatCurrencyForCSV } from '@/utils/csvUtils';

const FundInvestment = () => {
    const { theme } = useTheme();

    const [reportData, setReportData] = useState<FundInvestmentSummary[]>([]);
    ...
    const [filters, setFilters] = useState<FundInvestmentFilters>({});
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                ...
                const defaultFilters = await getUserDefaultLocationForFundInvestment();

                // Set the initial filters to user's default location
                setFilters(defaultFilters);

                // Load fund investment summary with user's default location filters
                const data = await getFundInvestmentSummary(defaultFilters);
                setReportData(data);
            } catch (err) {
                setError(err instanceof Error ? err.message : 'Failed to load data');
                console.error('Error loading fund investment report:', err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleFilterChange = async (newFilters: FundInvestmentFilters) => {
        try {
            setLoading(true);
            setFilters(newFilters);
            const data = await getFundInvestmentSummary(newFilters);
            setReportData(data);
        } catch (err) {
    ...
    const updateFilter = (key: string, value: string) => {
        ...
        if (value === '') {
            delete newFilters[key as keyof FundInvestmentFilters];
        } else {
            if (key === 'mainProgram') {
                if (value === 'NP' || value === 'ADB' || value === 'WB') {
                    newFilters[key as keyof FundInvestmentFilters] = value as any;
                }
            } else {
                newFilters[key as keyof FundInvestmentFilters] = value as any;
            }
    ...
    const filteredData = reportData.filter(item =>
        ...
        item.hhNumber?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.mainProgram?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.beneficiaryType?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    const exportToCSV = () => {
        ...
        try {
            ...
            const headers = [
                ...
                'HH Number',
                'NIC',
                'Government Contribution (LKR)',
                'Beneficiary Contribution (LKR)',
                'Bank Loan (LKR)',
                'Other Organization Contribution (LKR)',
                'Total Investment (LKR)',
            ];
            ...
            const csvContent = [
                ...
                ...filteredData.map(item => [
                    `"${cleanCSVValue(item.beneficiaryName)}"`,
                    `"${cleanCSVValue(item.hhNumber)}"`,
                    `"${cleanCSVValue(item.nic)}"`,
                    formatCurrencyForCSV(item.governmentContribution),
                    formatCurrencyForCSV(item.beneficiaryContribution),
                    formatCurrencyForCSV(item.bankLoan),
                    formatCurrencyForCSV(item.otherOrganizationContribution),
                    formatCurrencyForCSV(item.total),
                ].join(','))
            ].join('\n');
            ...
            if (link.download !== undefined) {
                ...
                link.setAttribute('download', `fund_investment_report${filterSuffix}_${timestamp}.csv`);
    ...
    if (loading) {
        return (
            <div className={`flex items-center justify-center min-h-screen ${getBgColor()}`}>
                ...
                <span className={`ml-3 text-lg ${getTextColor()}`}>Loading fund investment data...</span>
            </div>
        );
    }
            ...
            <div className={`rounded-lg shadow-sm p-6 mb-6 ${getCardBgColor()} ${getBorderColor()} border`}>
                <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                        <h1 className={`text-2xl font-bold mb-2 ${getTextColor()}`}>Fund Investment Report</h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex-1">
                        <input
                            type="text"
                            placeholder="Search by beneficiary name, NIC, HH number, program, etc..."
                            value={searchTerm}
