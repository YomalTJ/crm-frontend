...
interface OwnerDemographicsTableProps {
    data: (OwnerDemographicsItem & { action?: React.ReactNode })[];
    totalCount: number;
    showActionColumn?: boolean;
}

const OwnerDemographicsTable: React.FC<OwnerDemographicsTableProps> = ({
    data,
    totalCount,
    ...
    return (
        <div className={`rounded-lg shadow-sm overflow-hidden ${getCardBgColor()} ${getBorderColor()} border`}>
            <div className={`px-6 py-4 border-b ${getBorderColor()}`}>
                <h2 className={`text-lg font-medium ${getTextColor()}`}>Owner Demographics</h2>
                            ...
                            {data.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        {item.disability ? (
                                            <div className={`text-sm ${theme === 'dark' ? 'text-gray-200' : 'text-gray-900'}`}>
                                                <div className="font-medium">{item.disability.nameEN}</div>
                                                <div className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                                                    {item.disability.nameSi}
                                                </div>
                                            </div>
                                        ) : (
                                            <span className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
...
export default OwnerDemographicsTable;